OT_economicData = [
    [[8211.58,10929.5,0],"Edessa Power Plant"],
    [[5475.61,14983.5,0],"Stadium"],
    [[4228.01,15059.8,0],"Magos Power Plant"],
    [[21319.1,10421.5,0],"Surf Club"],
    [[16596.7,12764.5,0],"Altis Financial Services"],
    [[17092.5,11294.6,0],"Pyrgos Mines","","OT_Steel"],
    [[12146.1,8418.28,0],"Vikos Lumberyard","","OT_Wood"],
    [[17139.3,13050.5,0],"Pyrgos Vineyard","","OT_Grapes"],
    [[18123.6,13022.5,0],"Olive Plantation","","OT_Olives"],
    [[14305.6,18883.7,0],"Athira Wines","OT_Grapes","OT_Wine"]
    
];
OT_factoryPos = [6201.09,16257.1,0];
OT_factoryVehicleSpawn = [6183.15,16269.7,0];
OT_factoryVehicleDir = 67;
OT_connectedRegions = [
    
];
